...
//
//@group(0) @binding(3) var<uniform> iterators: array<Iterator, MAX_ITERATORS>;
//
//@group(0) @binding(4) var<uniform> alias_tables: array<vec4<f32>, MAX_ITERATORS>;
//
//@group(0) @binding(5) var<uniform> palette: array<vec4<f32>, MAX_PALETTE_COLORS>;
//
//@group(0) @binding(6) var<uniform> real_params: array<RealParam, MAX_PARAMS>;
//
//@group(0) @binding(7) var<uniform> vec3_params: array<vec4<f32>, MAX_PARAMS>;
//
...
//	let ipx = proj.x + proj.y * parameters.width;//pixel index
//	histogram[ipx] += color;
//}
...
//            any(isnan(p.pos.xyz)) //reset by plugin
		...
		if (settings.max_filter_radius > 0/* && proj.x>width/2*/) { //max filter radius <= aperture?
            ...
            if (settings.camera.projection_type == 1){
                nb.x = nb.x % parameters.width;
            }
            if (nb.x >= 0 && nb.x < parameters.width && nb.y >= 0 && nb.y < parameters.height) {
